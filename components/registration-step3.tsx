...
import { Droplets, User, ArrowLeft, AlertCircle, CheckCircle, Zap } from "lucide-react"
...
export function RegistrationStep3({ data, onUpdate, onComplete, onPrev }: RegistrationStep3Props) {
  ...
  const getDoseRecommendation = () => {
    ...
    if (ageInMonths >= 6 && ageInMonths <= 12) return { amount: "1", description: "६-१२ महिना" }
    if (ageInMonths > 12 && ageInMonths <= 24) return { amount: "2", description: "१-२ वर्ष" }
    if (ageInMonths > 24 && ageInMonths <= 60) return { amount: "4", description: "२-५ वर्ष" }
  ...
  const validateStep = () => {
    ...
    if (!data.administeredBy.trim()) newErrors.administeredBy = "सेवन गराउने व्यक्तिको नाम आवश्यक छ"
    if (!data.eligibilityConfirmed) newErrors.eligibilityConfirmed = "योग्यता पुष्टि गर्नुहोस्"
    ...
    <Card className="w-full max-w-2xl mx-auto">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-2 p-3 bg-amber-50 rounded-lg">
          ...
          <span className="text-sm font-medium text-amber-800">अन्तिम चरण | Final Step</span>
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="text-blue-800">{calculateAge(data.dateOfBirth)}</p>
              </div>
              <div>
                <span className="font-medium">लिङ्ग:</span>
                <p className="text-blue-800">{data.gender === "male" ? "पुरुष" : "महिला"}</p>
              </div>
              <div>
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="p-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-green-700">उमेर समूह: {doseRecommendation.description}</p>
                <p className="text-lg font-bold text-green-900">सिफारिस मात्रा: {doseRecommendation.amount} थोपा</p>
              </div>
              <div className="text-right">
                <div className="flex items-center gap-1">
                  {Array.from({ length: Number.parseInt(doseRecommendation.amount) }, (_, i) => (
                    <div key={i} className="w-3 h-3 bg-amber-400 rounded-full"></div>
                  ))}
                </div>
                <p className="text-xs text-green-600 mt-1">{doseRecommendation.amount} drops</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-4">
          <h3 className="font-semibold text-gray-900">प्रशासन विवरण | Administration Details</h3>
        ...
        <div className="space-y-4">
          <h3 className="font-semibold text-gray-900">सहमति र योग्यता | Consent & Eligibility</h3>
          ...
          <div className="space-y-3">
            <div className="flex items-start space-x-3">
              <Checkbox
                ...
                onCheckedChange={(checked) => onUpdate({ eligibilityConfirmed: checked })}
                className={errors.eligibilityConfirmed ? "border-red-500" : ""}
              />
              <Label htmlFor="eligibilityConfirmed" className="text-sm leading-relaxed cursor-pointer">
                म पुष्टि गर्छु कि यो बालक/बालिका स्वर्णबिन्दु प्राशनका लागि योग्य छ र कुनै निषेधित अवस्था छैन।
              </Label>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <Label htmlFor="consentGiven" className="text-sm leading-relaxed cursor-pointer">
                अभिभावकको सहमति लिइएको छ र स्वर्णबिन्दु प्राशनका सबै नियम र सावधानीहरू बुझाइएको छ।
              </Label>
            </div>
            {errors.consentGiven && (
              <p className="text-red-500 text-sm flex items-center gap-1 ml-6">
