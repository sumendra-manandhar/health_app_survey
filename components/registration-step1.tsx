...
export function RegistrationStep1({ data, onUpdate, onNext }: RegistrationStep1Props) {
  ...
  const [ageInfo, setAgeInfo] = useState<{ years: number; months: number; eligible: boolean } | null>(null)

  // Quick fill suggestions based on common names
  const commonNames = {
    male: ["राम", "श्याम", "हरि", "कृष्ण", "गोपाल", "सुरेश", "राजेश", "दिनेश"],
    female: ["सीता", "गीता", "राधा", "सरस्वती", "लक्ष्मी", "पार्वती", "दुर्गा", "कमला"],
  }

  const commonFatherNames = ["राम बहादुर", "कृष्ण बहादुर", "हरि प्रसाद", "गोपाल", "सुरेश", "राजेश"]
  const commonMotherNames = ["सीता", "गीता", "राधा", "कमला", "सरस्वती", "लक्ष्मी"]
  ...
  const validateStep = () => {
    ...
    if (!data.contactNumber.trim()) newErrors.contactNumber = "सम्पर्क नम्बर आवश्यक छ"
    ...
    <Card className="w-full max-w-2xl mx-auto">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg">
          ...
          <span className="text-sm font-medium text-blue-800">द्रुत दर्ता | Quick Registration</span>
        ...
        <div className="space-y-2">
          ...
          {data.gender && (
            <div className="flex flex-wrap gap-1 mt-2">
              ...
              {commonNames[data.gender as keyof typeof commonNames]?.slice(0, 4).map((name) => (
                <Button
                  key={name}
                  variant="outline"
                  size="sm"
        ...
        <div className="space-y-2">
          ...
          {ageInfo && (
            <div
              className={`p-3 rounded-lg ${ageInfo.eligible ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
            >
              <div className="flex items-center gap-2">
                <Calendar className={`h-4 w-4 ${ageInfo.eligible ? "text-green-600" : "text-red-600"}`} />
                <span className={`text-sm font-medium ${ageInfo.eligible ? "text-green-800" : "text-red-800"}`}>
                  उमेर: {ageInfo.years} वर्ष {ageInfo.months} महिना
                </span>
                <Badge variant={ageInfo.eligible ? "default" : "destructive"} className="ml-auto">
                  {ageInfo.eligible ? "योग्य" : "अयोग्य"}
                </Badge>
              </div>
              {!ageInfo.eligible && (
                <p className="text-red-700 text-xs mt-1">
                  स्वर्णबिन्दु प्राशन ६ महिनादेखि ५ वर्षसम्मका बालबालिकाका लागि मात्र हो।
                </p>
              )}
            </div>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => onUpdate({ fatherName: e.target.value, guardianName: e.target.value })}
              placeholder="बुबाको नाम"
            />
            <div className="flex flex-wrap gap-1">
              <span className="text-xs text-gray-600">सुझाव:</span>
              {commonFatherNames.slice(0, 3).map((name) => (
                <Button
                  key={name}
                  variant="outline"
                  size="sm"
                  className="h-6 text-xs bg-transparent"
                  onClick={() => quickFillParent(name, "father")}
                >
                  {name}
                </Button>
              ))}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              <span className="text-xs text-gray-600">सुझाव:</span>
              {commonMotherNames.slice(0, 3).map((name) => (
                <Button
                  key={name}
                  variant="outline"
                  size="sm"
                  className="h-6 text-xs bg-transparent"
                  onClick={() => quickFillParent(name, "mother")}
                >
                  {name}
                </Button>
              ))}
        ...
        <div className="space-y-2">
          ...
          <div className="flex">
            ...
            <Input
              ...
              onChange={(e) => onUpdate({ contactNumber: e.target.value.replace(/\D/g, "").slice(0, 10) })}
        ...
        {ageInfo && !ageInfo.eligible && (
          <Alert className="border-red-200 bg-red-50">
            ...
            <AlertDescription className="text-red-800">
              <strong>अयोग्य:</strong> स्वर्णबिन्दु प्राशन कार्यक्रम ६ महिनादेखि ५ वर्षसम्मका बालबालिकाका लागि मात्र हो।
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="flex justify-end pt-4">
          <Button onClick={handleNext} disabled={ageInfo && !ageInfo.eligible} className="px-8">
            अर्को चरण | Next Step
          </Button>
        </div>
      </CardContent>
