...
import { Wifi, WifiOff, RefreshCw, CheckCircle, AlertCircle, Upload, Clock } from "lucide-react"
...
export function SyncManager() {
  ...
  const handleAutoSync = async () => {
    ...
    try {
      ...
      setSyncMessage(`Syncing ${pendingRegistrations.length} registrations and ${pendingScreenings.length} screenings`)
      setSyncProgress(25)
      ...
      if (result.success) {
        ...
        pendingRegistrations.forEach((reg) => OfflineStorage.markRegistrationSynced(reg.id))
        pendingScreenings.forEach((screening) => OfflineStorage.markScreeningSynced(screening.id))
  ...
  return (
    <Card className="w-full">
      <CardHeader className="pb-3">
        ...
        <CardDescription>{syncStatus.isOnline ? "Connected to server" : "Working offline"}</CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Badge variant={syncStatus.pendingCount > 0 ? "secondary" : "outline"}>{syncStatus.pendingCount}</Badge>
        </div>
        ...
        {!syncStatus.isOnline && (
          <Alert>
            ...
            <AlertDescription>
              You're working offline. Data will be saved locally and synced when connection is restored.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
