...
import { Download, Search, Users, AlertTriangle, CheckCircle, TrendingUp, Droplets } from "lucide-react"
...
export default function ReportsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const localScreenings = OfflineStorage.getOfflineScreenings().map((s) => s.data)
  ...
  const filterData = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (record) =>
          ...
          record.guardian_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    if (reactionFilter !== "all") {
      filtered = filtered.filter((record) => record.child_reaction === reactionFilter)
    }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `swarnabindu-report-${new Date().toISOString().split("T")[0]}.csv`)
  ...
  const getStatistics = () => {
    ...
    const normalReactions = screenings.filter((s) => s.child_reaction === "normal").length
    const adverseReactions = screenings.filter((s) => s.child_reaction === "adverse").length

    // Age groups based on dose amounts
    const ageGroups = {
      "6-12 महिना": registrations.filter((r) => r.dose_amount === "1" || r.dose_amount === "2").length,
      "1-2 वर्ष": registrations.filter((r) => r.dose_amount === "3" || r.dose_amount === "4").length,
      "2-5 वर्ष": registrations.filter((r) => Number.parseInt(r.dose_amount) >= 4).length,
    }
    ...
    const monthlyData = registrations.reduce(
      (acc, reg) => {
        const month = new Date(reg.date).toLocaleDateString("ne-NP", { year: "numeric", month: "short" })
  ...
  const monthlyTrendData = Object.entries(stats.monthlyData).map(([name, value]) => ({
    name,
    registrations: value,
  }))

  const uniqueDistricts = [...new Set(registrations.map((r) => r.district))].filter(Boolean)
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">रिपोर्ट र विश्लेषण</h1>
              <p className="text-gray-600">स्वर्णबिन्दु प्राशन कार्यक्रम डाटा विश्लेषण</p>
            </div>
            <Link href="/">
            ...
            <Card className="border-blue-200 bg-blue-50">
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-blue-600">कुल दर्ता</p>
            ...
            <Card className="border-purple-200 bg-purple-50">
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-purple-600">कुल स्क्रिनिङ</p>
                    <p className="text-2xl font-bold text-purple-900">{stats.totalScreenings}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-green-200 bg-green-50">
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-green-600">सामान्य प्रतिक्रिया</p>
                    <p className="text-2xl font-bold text-green-900">{stats.normalReactions}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-red-200 bg-red-50">
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-red-600">प्रतिकूल प्रतिक्रिया</p>
                    <p className="text-2xl font-bold text-red-900">{stats.adverseReactions}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-amber-200 bg-amber-50">
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-amber-600">सफलता दर</p>
                    <p className="text-2xl font-bold text-amber-900">
                      {stats.totalScreenings > 0
                        ? Math.round((stats.normalReactions / stats.totalScreenings) * 100)
                        : 0}
                      %
                    </p>
                  </div>
                    ...
                    <TableBody>
                      {filteredData.map((record) => (
                        <TableRow key={record.id}>
                          <TableCell className="font-medium">{record.serial_no}</TableCell>
                          <TableCell>{record.child_name}</TableCell>
                          <TableCell>{record.age}</TableCell>
                          <TableCell>
                            <Badge variant="outline">{record.gender === "male" ? "पुरुष" : "महिला"}</Badge>
                          </TableCell>
                          ...
                          <TableCell>
                            <Badge variant={record.child_reaction === "normal" ? "default" : "destructive"}>
                              {record.child_reaction === "normal" ? "सामान्य" : "प्रतिक्रिया"}
                            </Badge>
                          </TableCell>
                        </TableRow>
                    ...
                    <TableBody>
                      {screenings.map((screening, index) => (
                        <TableRow key={screening.id}>
                          ...
                          <TableCell>{new Date(screening.screening_date).toLocaleDateString("ne-NP")}</TableCell>
                          <TableCell className="font-mono text-xs">{screening.patient_id}</TableCell>
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <div className="flex items-center gap-1">
                                {Array.from({ length: Number.parseInt(screening.dose_amount) || 1 }, (_, i) => (
                                  <div key={i} className="w-2 h-2 bg-amber-400 rounded-full"></div>
                                ))}
                              </div>
                              <span className="text-sm">{screening.dose_amount} थोपा</span>
                            </div>
                          </TableCell>
                          <TableCell className="text-sm">{screening.administered_by}</TableCell>
                          <TableCell>
                            <Badge variant={screening.child_reaction === "normal" ? "default" : "destructive"}>
                              {screening.child_reaction === "normal" ? "सामान्य" : "प्रतिक्रिया"}
                            </Badge>
                          </TableCell>
                          <TableCell className="font-mono text-xs">{screening.batch_number}</TableCell>
                          <TableCell className="text-xs max-w-32 truncate">{screening.notes || "-"}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                      <div>
                        <p className="text-sm font-medium text-green-800">सामान्य प्रतिक्रिया</p>
                        <p className="text-2xl font-bold text-green-900">{stats.normalReactions}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-green-600">
                          {stats.totalScreenings > 0
                            ? ((stats.normalReactions / stats.totalScreenings) * 100).toFixed(1)
                            : 0}
                          %
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-red-50 rounded-lg">
                      <div>
                        <p className="text-sm font-medium text-red-800">प्रतिकूल प्रतिक्रिया</p>
                        <p className="text-2xl font-bold text-red-900">{stats.adverseReactions}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-red-600">
                          {stats.totalScreenings > 0
                            ? ((stats.adverseReactions / stats.totalScreenings) * 100).toFixed(1)
                            : 0}
                          %
                        </p>
                      </div>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="text-center">
                    ...
                    <div className="text-sm text-gray-600">कुल दर्ता संख्या</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-purple-600">{stats.totalScreenings}</div>
                  ...
                  <div className="text-center">
                    <div className="text-3xl font-bold text-red-600">
                      {stats.totalScreenings > 0
                        ? Math.round((stats.adverseReactions / stats.totalScreenings) * 100)
                        : 0}
                      %
                    </div>
                    <div className="text-sm text-gray-600">प्रतिकूल प्रतिक्रिया दर</div>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="trends" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Card>
                ...
                <CardContent className="text-center">
                  <div className="text-4xl font-bold text-purple-600 mb-2">
                    {stats.total > 0 ? (stats.totalScreenings / stats.total).toFixed(1) : 0}
                  </div>
              ...
              <Card>
                ...
                <CardContent className="text-center">
                  <div className="text-4xl font-bold text-green-600 mb-2">{uniqueDistricts.length}</div>
