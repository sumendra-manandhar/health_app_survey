...
export default function NewScreeningPage() {
  ...
  const loadPatient = async () => {
    ...
    try {
      ...
      const offlinePatient = offlineRegistrations.find((reg) => reg.localId === patientId || reg.id === patientId)
      ...
      if (offlinePatient) {
        setPatient({
          ...
          guardian_name: offlinePatient.guardian_name || "",
  ...
  const loadDoseHistory = async () => {
    try {
      ...
      if (isOnline) {
        try {
          const { data, error } = await DatabaseService.getScreenings(patientId)
          if (data && !error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        try {
          const { data, error } = await DatabaseService.createScreening(screeningRecord)
          if (!error) {
    ...
    return (
      <div className="container mx-auto p-6">
        <Alert>
          ...
          <AlertDescription>Patient not found. Please go back and select a patient for screening.</AlertDescription>
        </Alert>
        <div className="mt-4">
          <Link href="/screening">
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Conduct follow-up screening for patient</p>
        </div>
      </div>
      ...
      {!isOnline && (
        <Alert>
          <AlertDescription>
            You're working offline. Screening data will be saved locally and synced when online.
          </AlertDescription>
        </Alert>
      )}
        ...
        <div className="lg:col-span-1 space-y-6">
              ...
              {patient ? (
                <>
                  <div>
                    ...
                    <p className="text-lg font-semibold">{patient.child_name}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-600">सिरियल नम्बर:</Label>
                    <p className="font-mono">{patient.serial_no}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="text-sm font-medium text-gray-600">उमेर:</Label>
                      <p>{patient.age}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-600">लिङ्ग:</Label>
                      <Badge variant="outline">{getGenderDisplay(patient.gender)}</Badge>
                    </div>
                  </div>
                  <Separator />
                  <div>
                    <Label className="text-sm font-medium text-gray-600">अभिभावक:</Label>
                    <p>{patient.guardian_name || patient.father_name || patient.mother_name}</p>
                  </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Patient details not fully loaded</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-green-200 bg-green-50">
            <CardHeader>
              ...
              <CardDescription>
                कुल मात्रा: {doseHistory.length} | Total Doses: {doseHistory.length}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* Progress Bar */}
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {doseHistory.length > 0 ? (
                  doseHistory.map((dose, index) => (
                    <div key={dose.id} className="p-3 bg-white rounded-lg border border-green-200">
                      <div className="flex items-center justify-between mb-2">
                        <Badge variant="outline" className="text-xs">
                          मात्रा #{doseHistory.length - index}
                        </Badge>
                        <span className="text-xs text-gray-600">{formatDate(dose.date)}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          <span className="font-medium">मात्रा:</span>
                          <div className="flex items-center gap-1 mt-1">
                            {Array.from({ length: Number.parseInt(dose.dose_amount) || 1 }, (_, i) => (
                              <div key={i} className="w-2 h-2 bg-blue-500 rounded-full"></div>
                            ))}
                            <span className="ml-1">{dose.dose_amount} थोपा</span>
                          </div>
                        </div>
                        <div>
                          <span className="font-medium">प्रतिक्रिया:</span>
                          <div className="mt-1">{getReactionBadge(dose.reaction)}</div>
                        </div>
                      </div>
                      {dose.notes && <p className="text-xs text-gray-600 mt-2 italic">{dose.notes}</p>}
                    </div>
                  ))
                ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div className="text-center">
                      ...
                      <p className="text-green-700">
                        {doseHistory.filter((d) => d.reaction === "normal").length}/{doseHistory.length}
                      </p>
                    </div>
                    <div className="text-center">
                      ...
                      <p className="text-blue-700">
                        {doseHistory.length > 0 ? formatDate(doseHistory[0].date) : "N/A"}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <form onSubmit={handleSubmit}>
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleInputChange("screening_date", e.target.value)}
                      required
                    />
                  </div>
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleInputChange("screening_type", value)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          onChange={(e) => handleInputChange("dose_amount", e.target.value)}
                          placeholder="2-6 थोपा"
                          required
                        />
                        <div className="flex items-center gap-1">
                          {Array.from({ length: Number.parseInt(screeningData.dose_amount) || 1 }, (_, i) => (
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => handleInputChange("dose_time", e.target.value)}
                        required
                      />
                    </div>
                    <div>
                      <Label htmlFor="administered_by">सेवन गराउने व्यक्ति *</Label>
                      <Input
                        id="administered_by"
                ...
                <div>
                  <h3 className="text-lg font-semibold mb-4">शारीरिक मापदण्ड</h3>
                ...
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="child_reaction">बच्चाको प्रतिक्रिया *</Label>
                    <Select
                      value={screeningData.child_reaction}
                      onValueChange={(value) => handleInputChange("child_reaction", value)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        ...
                        <SelectItem value="mild_reaction">हल्का प्रतिक्रिया (Mild Reaction)</SelectItem>
                        <SelectItem value="adverse">प्रतिकूल प्रतिक्रिया (Adverse Reaction)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
      ...
      <Card className="border-yellow-200 bg-yellow-50">
        <CardContent className="p-6">
          ...
          <ul className="text-sm text-yellow-700 space-y-1">
            <li>• स्वर्णप्राशन दिनु अघि बच्चाको स्वास्थ्य अवस्था जाँच गर्नुहोस्</li>
